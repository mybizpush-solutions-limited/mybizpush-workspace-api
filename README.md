...
The whole integration is **one GitHub App** installed on the org — no personal access tokens, one org-wide webhook.

- **Create + install:** create a **GitHub App** under the org (Settings → Developer settings → GitHub Apps), grant **Pull requests: Read-only**, **Contents: Read-only**, and **Organization → Members: Read-only**, subscribe to **Pull request** events, then **Install** it on the org. Put `GITHUB_APP_ID` + `GITHUB_APP_PRIVATE_KEY` (the `.pem`, with literal `\n` newlines or base64-encoded) and `GITHUB_ORG` in `.env`. The App reads PRs/repos org-wide via a short-lived installation token (auto-minted from a signed JWT and cached) — no personal access token. Link repos to a project from **Project → Repos**.
...
- **SSL.** Hosted Postgres is TLS-only, so a connection string that says nothing about SSL is assumed to need it. Self-hosted and LAN servers are often built without TLS and answer `the server does not support SSL connections` — flip the **Use SSL** toggle off (or send `ssl: false` on create/patch) and the stored `ssl_mode` becomes the authority for both the connection probe and `pg_dump`. An explicit `?sslmode=` in the URL is respected when no toggle is sent, and turning SSL back *on* preserves a stricter mode like `verify-full` rather than relaxing it to plain `require`.
